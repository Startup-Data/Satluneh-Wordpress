...
class Less_Parser {
	...
	private function parseEntitiesQuoted() {
		...
		return $this->NewObj5( 'Less_Tree_Quoted', array(
			$quoted,
			$matched,
			$e,
			$index,
	...
	private function parseEntitiesUrl() {
		...
		return $this->NewObj2( 'Less_Tree_Url', array(
			$this->NewObj1( 'Less_Tree_Anonymous', $value ),
			$this->env->currentFileInfo
		) );
	}
	...
	private function parseEntitiesVariableCurly() {
		...
		if ( $this->input_len > ( $this->pos + 1 ) && $this->input[ $this->pos ] === '@' && ( $curly = $this->MatchReg( '/\\G@\{([\w-]+)\}/' ) ) ) {
			return $this->NewObj3( 'Less_Tree_Variable', array(
				'@' . $curly[1],
				$index,
				$this->env->currentFileInfo
			) );
	...
	function parseExtend( $isRule = false ) {
		...
		do {
			...
			$extendList[] = $this->NewObj3( 'Less_Tree_Extend', array(
				$this->NewObj1( 'Less_Tree_Selector', $elements ),
				$option,
				$index
			) );
	...
	private function parseMixinCall() {
		...
		if ( $elements ) {
			...
			if ( $this->parseEnd() ) {
				...
				return $this->NewObj5( 'Less_Tree_Mixin_Call', array(
					$elements,
					$args,
					$index,
					$this->env->currentFileInfo,
	...
	private function parseMixinCallElements() {
		...
		while ( true ) {
			...
			$elements[] = $this->NewObj4( 'Less_Tree_Element', array(
				$c,
				$e[0],
				$elemIndex,
				$this->env->currentFileInfo
	...
	private function parseMixinDefinition() {
		...
		if ( $match ) {
			...
			if ( is_array( $ruleset ) ) {
				...
				return $this->NewObj5( 'Less_Tree_Mixin_Definition', array(
					$name,
					$params,
					$ruleset,
					$cond,
	...
	private function parseSelector( $isLess = false ) {
		...
		if ( $elements ) {
			return $this->NewObj5( 'Less_Tree_Selector', array(
				$elements,
				$extendList,
				$condition,
				$index,
	...
	private function parseRuleset() {
		...
		if ( $selectors ) {
			...
			if ( is_array( $rules ) ) {
				...
				return $this->NewObj2( 'Less_Tree_Ruleset', array(
					$selectors,
					$rules
				) ); //Less_Environment::$strictImports
			}
		}
	...
	private function parseRule( $tryAnonymous = null ) {
		...
		if ( $name ) {
			...
			if ( $value && $this->parseEnd() ) {
				...
				return $this->NewObj6( 'Less_Tree_Rule', array(
					$name,
					$value,
					$important,
					$merge,
		...
		if ( $dir ) {
			...
			if ( $path ) {
				...
				if ( $this->MatchChar( ';' ) ) {
					...
					return $this->NewObj5( 'Less_Tree_Import', array(
						$path,
						$features,
						$options,
						$this->pos,
	...
	private function parseMediaFeature() {
		...
		do {
			...
			if ( $e ) {
				...
				if ( $this->MatchChar( ')' ) ) {
					if ( $p && $e ) {
						$r       = $this->NewObj7( 'Less_Tree_Rule', array(
							$p,
							$e,
							null,
							null,
	...
	private function parseMedia() {
		if ( $this->MatchReg( '/\\G@media/' ) ) {
			...
			if ( is_array( $rules ) ) {
				return $this->NewObj4( 'Less_Tree_Media', array(
					$rules,
					$features,
					$this->pos,
					$this->env->currentFileInfo
	...
	private function parseDirective() {
		...
		if ( $rules || ( ! $hasBlock && $value && $this->MatchChar( ';' ) ) ) {
			...
			return $this->NewObj5( 'Less_Tree_Directive', array(
				$name,
				$value,
				$rules,
				$index,
	...
	private function parseSub() {
		...
		if ( $this->MatchChar( '(' ) ) {
			...
			if ( $a ) {
				...
				return $this->NewObj2( 'Less_Tree_Expression', array(
					array( $a ),
					true
	...
	private function parseCondition() {
		...
		if ( $a ) {
			...
			return $this->MatchReg( '/\\Gand/' ) ? $this->NewObj3( 'Less_Tree_Condition', array(
				'and',
				$c,
				$this->parseCondition()
			) ) : $c;
		...
		if ( ( count( $name ) > 1 ) && $this->rulePropertyMatch( '/\\G\s*((?:\+_|\+)?)\s*:/', $offset, $length, $index, $name ) ) {
			...
			foreach ( $name as $k => $s ) {
				if ( ! $s || $s[0] !== '@' ) {
					...
					$name[ $k ] = $this->NewObj3( 'Less_Tree_Variable', array(
						'@' . substr( $s, 2, - 1 ),
						$index[ $k ],
						$this->env->currentFileInfo
					) );

...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
	...
	public function show_field_image( $field, $meta ) {
		...
		echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
		if ($has_image)
		echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".__('Remove Image', 'tax-meta')."' type='button'/>";
		else
		echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".__('Upload Image', 'tax-meta')."' type='button'/>";
		$this->show_field_end( $field, $meta );
	}
	...
	public function footer_js(){
		...
		jQuery(document).ready(function(){
			...
			function clear_form_meta(){
					...
					//clear checkbox
					jQuery("#addtag input:checkbox").prop("checked", false);
					//clear radio buttons

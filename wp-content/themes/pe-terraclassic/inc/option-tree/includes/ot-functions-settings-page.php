...
if ( ! function_exists( 'ot_type_theme_options_ui' ) ) {
	...
	function ot_type_theme_options_ui() {
		...
		echo '<h4>' . __( 'Warning!', 'pe-terraclassic' ) . '</h4>';
		echo '<p class="warning">' . sprintf( __( 'Go to the %s page if you want to save data, this page is for adding settings.', 'pe-terraclassic' ), '<a href="' . get_admin_url( $blog_id, apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) . '?page=' . apply_filters( 'ot_theme_options_menu_slug', 'ot-theme-options' ) ) . '"><code>Appearance->Theme Options</code></a>' ) . '</p>';
		echo '<p class="warning">' . sprintf( __( 'If you\'re unsure or not completely positive that you should be editing these settings, you should read the %s first.', 'pe-terraclassic' ), '<a href="' . get_admin_url( $blog_id, 'admin.php?page=ot-documentation' ) . '"><code>OptionTree->Documentation</code></a>' ) . '</p>';
		echo '<h4>' . __( 'Things could break or be improperly displayed to the end-user if you do one of the following:', 'pe-terraclassic' ) . '</h4>';
		echo '<p class="warning">' . __( 'Give two sections the same ID, give two settings the same ID, give two contextual help content areas the same ID, don\'t create any settings, or have a section at the end of the settings list.', 'pe-terraclassic' ) . '</p>';
		echo '<p>' . __( 'You can create as many settings as your project requires and use them how you see fit. When you add a setting here, it will be available on the Theme Options page for use in your theme. To separate your settings into sections, click the "Add Section" button, fill in the input fields, and a new navigation menu item will be created.', 'pe-terraclassic' ) . '</p>';
		echo '<p>' . __( 'All of the settings can be sorted and rearranged to your liking with Drag & Drop. Don\'t worry about the order in which you create your settings, you can always reorder them.', 'pe-terraclassic' ) . '</p>';
		...
		/* buttons */
		echo '<a href="javascript:void(0);" class="option-tree-section-add option-tree-ui-button button hug-left">' . __( 'Add Section', 'pe-terraclassic' ) . '</a>';
		echo '<a href="javascript:void(0);" class="option-tree-setting-add option-tree-ui-button button">' . __( 'Add Setting', 'pe-terraclassic' ) . '</a>';
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . __( 'Save Changes', 'pe-terraclassic' ) . '</button>';

		/* sidebar textarea */
		echo '
          <div class="format-setting-label" id="contextual-help-label">
            <h3 class="label">' . __( 'Contextual Help', 'pe-terraclassic' ) . '</h3>
          </div>
          <div class="format-settings" id="contextual-help-setting">
            <div class="format-setting type-textarea no-desc">
              <div class="description"><strong>' . __( 'Contextual Help Sidebar', 'pe-terraclassic' ) . '</strong>: ' . __( 'If you decide to add contextual help to the Theme Option page, enter the optional "Sidebar" HTML here. This would be an extremely useful place to add links to your themes documentation or support forum. Only after you\'ve added some content below will this display to the user.', 'pe-terraclassic' ) . '</div>
		...
		echo '<a href="javascript:void(0);" class="option-tree-help-add option-tree-ui-button button hug-left">' . __( 'Add Contextual Help Content', 'pe-terraclassic' ) . '</a>';
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . __( 'Save Changes', 'pe-terraclassic' ) . '</button>';
...
if ( ! function_exists( 'ot_type_import_xml' ) ) {
	...
	function ot_type_import_xml() {
		...
		echo '<p class="deprecated">' . __( 'This import method has been deprecated. That means it has been replaced by a new method and is no longer supported, and may be removed from future versions. All themes that use this import method should be converted to use its replacement below.', 'pe-terraclassic' ) . '</p>';

		echo '<p>' . sprintf( __( 'If you were given a Theme Options XML file with a premium or free theme, locate it on your hard drive and upload that file by clicking the upload button. A popup window will appear, upload the XML file and click "%s". The file URL should be in the upload input, if it is click "Import XML".', 'pe-terraclassic' ), apply_filters( 'ot_upload_text', __( 'Send to OptionTree', 'pe-terraclassic' ) ) ) . '</p>';

		/* button */
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . __( 'Import XML', 'pe-terraclassic' ) . '</button>';
		...
		/* add xml button */
		echo '<a href="javascript:void(0);" class="ot_upload_media option-tree-ui-button button button-primary light" rel="' . $post_id . '" title="' . __( 'Add XML', 'pe-terraclassic' ) . '"><span class="icon ot-icon-plus-circle"></span>' . __( 'Add XML', 'pe-terraclassic' ) . '</a>';
...
if ( ! function_exists( 'ot_type_import_settings' ) ) {
	...
	function ot_type_import_settings() {
		...
		echo '<p>' . __( 'To import your Settings copy and paste what appears to be a random string of alpha numeric characters into this textarea and press the "Import Settings" button.', 'pe-terraclassic' ) . '</p>';

		/* button */
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . __( 'Import Settings', 'pe-terraclassic' ) . '</button>';
...
if ( ! function_exists( 'ot_type_import_data' ) ) {
	...
	function ot_type_import_data() {
		...
		if ( OT_SHOW_SETTINGS_IMPORT ) {
			echo '<p>' . __( 'Only after you\'ve imported the Settings should you try and update your Theme Options.', 'pe-terraclassic' ) . '</p>';
		}

		echo '<p>' . __( 'To import your Theme Options copy and paste what appears to be a random string of alpha numeric characters into this textarea and press the "Import Theme Options" button.', 'pe-terraclassic' ) . '</p>';

		/* button */
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . __( 'Import Theme Options', 'pe-terraclassic' ) . '</button>';
...
if ( ! function_exists( 'ot_type_import_layouts' ) ) {
	...
	function ot_type_import_layouts() {
		...
		if ( OT_SHOW_SETTINGS_IMPORT ) {
			echo '<p>' . __( 'Only after you\'ve imported the Settings should you try and update your Layouts.', 'pe-terraclassic' ) . '</p>';
		}

		echo '<p>' . __( 'To import your Layouts copy and paste what appears to be a random string of alpha numeric characters into this textarea and press the "Import Layouts" button. Keep in mind that when you import your layouts, the active layout\'s saved data will write over the current data set for your Theme Options.', 'pe-terraclassic' ) . '</p>';

		/* button */
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . __( 'Import Layouts', 'pe-terraclassic' ) . '</button>';
...
if ( ! function_exists( 'ot_type_export_settings_file' ) ) {
	...
	function ot_type_export_settings_file() {
		...
		echo '<p>' . sprintf( __( 'Export your Settings into a fully functional %s file. If you want to add your own custom %s text domain to the file, enter it into the text field before exporting. For more information on how to use this file read the documentation on %s. Remember, you should always check the file for errors before including it in your theme.', 'pe-terraclassic' ), '<code>theme-options.php</code>', '<a href="http://codex.wordpress.org/I18n_for_WordPress_Developers" target="_blank">I18n</a>', '<a href="' . get_admin_url( $blog_id, 'admin.php?page=ot-documentation#section_theme_mode' ) . '">' . __( 'Theme Mode', 'pe-terraclassic' ) . '</a>' ) . '</p>';
		...
		/* button */
		echo '<button class="option-tree-ui-button button button-primary hug-left">' . __( 'Export Settings File', 'pe-terraclassic' ) . '</button>';
...
if ( ! function_exists( 'ot_type_modify_layouts' ) ) {
	...
	function ot_type_modify_layouts() {
		...
		echo '<p>' . __( 'To add a new layout enter a unique lower case alphanumeric string (dashes allowed) in the text field and click "Save Layouts".', 'pe-terraclassic' ) . '</p>';
		echo '<p>' . __( 'As well, you can activate, remove, and drag & drop the order; all situations require you to click "Save Layouts" for the changes to be applied.', 'pe-terraclassic' ) . '</p>';
		echo '<p>' . __( 'When you create a new layout it will become active and any changes made to the Theme Options will be applied to it. If you switch back to a different layout immediately after creating a new layout that new layout will have a snapshot of the current Theme Options data attached to it.', 'pe-terraclassic' ) . '</p>';
		if ( OT_SHOW_DOCS ) {
			echo '<p>' . __( 'Visit <code>OptionTree->Documentation->Layouts Overview</code> to see a more in-depth description of what layouts are and how to use them.', 'pe-terraclassic' ) . '</p>';
		}
		...
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . __( 'Save Layouts', 'pe-terraclassic' ) . '</button>';
